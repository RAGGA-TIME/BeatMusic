<template>
    <div>
        <transition name="slidey-fadey">
            <div v-show="$store.state.showPlayer" id="Player" class="Player">
            <!-- playlist -->
            ...
            <div v-show="isPlayList" class="playlist_container">
                ...
                <div style="width:100vw; height:40vh; overflow-y: scroll; ">
                    <div style="height:15px; width:1px;"></div>
                    <div v-show="playList.length <= 0" style="margin-left:45vw;">No songs</div>
                    <div v-show="playList.length > 0" v-for="(item, index) in playList" :key="index">
                        <div style="width:100vw; height:66px; display:flex; ">
                            ...
                            <div style="margin-left:5vw;width:75vw; margin-top:20px;">{{item.name}} - {{item.author}}</div>
            ...
            <div v-show="isPlayList" @click="isPlayList = false" style="width:100vw; height:100vh; background-color:rgba(0,0,0, 0.45); z-index:101; position:absolute; "></div>

                    <audio @ended="playEnded" id="winPlay" :src="nowPlaying[0] ? nowPlaying[0].music_url : ''"></audio>
              ...
              <div id="mainCover" class="mainCover" v-if="nowPlaying[0]" :style="{ background:'url(' + nowPlaying[0].cover_url + ') no-repeat center 0/cover' }">
              </div>
              <div id="shadowx" class="shadowx" v-if="nowPlaying[0]" :style="{ background:'url(' + nowPlaying[0].cover_url + ') no-repeat center 0/cover' }">
              </div>

              <div style="width:100vw; text-align:center; position:absolute;; bottom:26.5vh; background-color:rgb(255,253,254);">
              ...
              <div style="width:100vw; height:70px; position:absolute; bottom:17vh; display:flex; background-color:rgb(255,253,254);">
              <span style="position:absolute; left:6.5vw; margin-top:40px;">{{ nowDuration }}</span>
                <div @mousedown="proDown" style="width:58vw; height:5px; background-color:#eeefef; margin-left:20vw; margin-top:47.5px; overflow: hidden;">
                  <div id="moveOn" style="width:0vw; height:5px; background-color:#2c2c2c;"></div>
                </div>
                <span style="position:absolute; right:7vw; margin-top:40px;">{{ duration }}</span>
              </div>
              <div style="width:100vw; height:70px; position:absolute; bottom:6vh;
              display:flex; justify-content: center; align-items: center; text-align:center; ">
                ...
                <!-- 播放暂停 -->
                  <transition name="bounce">
                      <img id="triggerPlay" @mouseenter="mousePlay = true" @mouseleave="mousePlay = false" v-if="!play" @click="openPlayer"
  ...
  mounted () {
    Bus.$on('getSongx', (data) => {
      ...
      this.playList.forEach((e, v) => {
        if (this.playList[v]._id === data._id) { result = false }
      })
    ...
    $('#mainCover').addClass('rotate-pause')
    $('#winPlay')[0].oncanplay = () => {
      $('#winPlay')[0].play()
      this.durationSeconds = $('#winPlay')[0].duration
      this.duration = this.countmusicTime(this.durationSeconds)
      if (this.play === true) { $('#winPlay')[0].play() }
    }
    $('#winPlay')[0].onplaying = () => {
      ...
      $('#mainCover').removeClass('rotate-pause').addClass('rotate-running')
    }
    $('#winPlay')[0].onpause = () => {
      this.play = false
      Bus.$emit('iconChange', require('../../../assets/icons/menu.png'))
      $('#mainCover').removeClass('rotate-running').addClass('rotate-pause')
    }
  },
  methods: {
    playEnded (e) {
    ...
    openPlayer () {
      ...
      $('#winPlay')[0].currentTime = this.nowDurationSeconds
      $('#winPlay')[0].play()
      this.runTime = setInterval(() => {
        this.nowDurationSeconds = $('#winPlay')[0].currentTime
        this.nowDuration = this.countmusicTime(this.nowDurationSeconds)
        let changeWidth = (this.nowDurationSeconds / this.durationSeconds) * 100 * 0.578
        $('#moveOn').css('width', changeWidth + 'vw')
      }, 500)
    },
    closePlayer () {
      $('#mainCover').toggleClass('rotate-pause')
      $('#winPlay')[0].pause()
    ...
    proDown (e) {
      if (e.button === 0) { // 判断是否点击鼠标左键
        ...
        //  计算变动时间
        let widthPoor = this.screenWidth * 0.2
        let allWidth = this.screenWidth * 0.6
        let nowPercent = (this.gapX - widthPoor) / allWidth
        this.nowDurationSeconds = this.durationSeconds * nowPercent
        this.nowDuration = this.countmusicTime(this.nowDurationSeconds)
        $('#winPlay')[0].currentTime = this.nowDurationSeconds
        $('#moveOn').css('width', this.gapX - widthPoor + 'px')
    ...
    proMove (e) {
      var left = e.clientX - this.gapX // 鼠标移动的相对距离
      $(window).scrollLeft(this.startx - left)
      //  计算变动时间
      let widthPoor = this.screenWidth * 0.2
      let allWidth = this.screenWidth * 0.6
      let nowPercent = ((this.gapX + left) - widthPoor) / allWidth
      this.nowDurationSeconds = this.durationSeconds * nowPercent
      this.nowDuration = this.countmusicTime(this.nowDurationSeconds)
      $('#moveOn').css('width', (this.gapX + left) - widthPoor + 'px')
      return false // 阻止默认事件或冒泡
    },
    proStop (e) {
      // 解绑定，这一步很必要，前面有解释
