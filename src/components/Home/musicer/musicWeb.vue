<template>
        <transition name="slide-fade">
            <div v-show="show" class="musicWeb">
                <!-- 放大 -->
                ...
                <!-- 常规大小 -->
                <div v-if="showNomalContext" id="musicShowNomalContext" class="showNomalContext">
                ...
                <div v-show="show" id="musicclap" class="musicclap"></div>
            </div>
        </transition>
</template>
...
export default {
  ...
  methods: {
    ...
    biger () {
      ...
      this.inside = false
      setTimeout(() => {
        $('#musicShowNomalContext').animate({ width: '45vw', height: '100vh', right: '0px', top: '0px' }, () => {
        ...
        $('#musicclap').css('position', 'fixed').animate({ width: '45vw', height: '100vh', right: '0px', top: '0px' })
      }, 400)
    },
    backTurn () {
      ...
      setTimeout(() => {
        $('#musicShowNomalContext').animate({ width: '190px', height: '240px', right: '80px', top: '160px' }, () => {
          this.inside = true
          this.$emit('getcancel', 'showmusic')
          $('#musicclap').css('position', 'absolute')
          $('#musicShowNomalContext').css({ right: '', top: '' })
          this.$store.state.isCanClickBackGround = true
        })
      }, 100)
      $('#musicclap').animate({ width: '190px', height: '240px', right: '0px', top: '0px' })
    },
    async getMyMusic () {
    ...
    async searchMusic () {
      let res = await this.$MusicHttp.SearchMusic()
      this.musics = res
    },
    async searchAlbum () {
      let res = await this.$MusicHttp.SearchAlbum()
      this.albums = res
    }
  }
}
...
.musicWeb {
    position:absolute; width:190px; height:240px; top:160px; right:80px; z-index:10; border-radius:8px;
    .showNomalContext {
